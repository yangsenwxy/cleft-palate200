...
#    newDir1=newDir+P+l1 
    ...
    xs = tf.placeholder(tf.float32, [None, 1536])   # 28x28
    ...
    x_image = tf.reshape(xs, [-1, 12, 128, 1])
    ...
    ## fc1 layer ##
    W_fc1 = weight_variable([3*32*64, 1024])
    b_fc1 = bias_variable([1024])
    # [n_samples, 7, 7, 64] ->> [n_samples, 7*7*64]
    h_pool2_flat = tf.reshape(h_pool2, [-1, 3*32*64])
    ...
    #          sess.run(train_step, feed_dict={xs: train_data2, ys: label2, keep_prob: 0.5})
            ...
            if i % 200 == 0:
              print(compute_accuracy(train_data,train_label))
...
pathsave5=r'E:\deepdata\hui\four.npy'
