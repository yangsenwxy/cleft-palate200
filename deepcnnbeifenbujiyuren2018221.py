...
s=np.array([])
s1=np.array([])
load_test_label = r'E:\ysdeeplearn\deepcode\deeptrain\sp1024\基于人的\3\test_label.mat'
...
load_train_label= r'E:\ysdeeplearn\deepcode\deeptrain\sp1024\基于人的\3\train_label.mat'
...
load_train_data= r'E:\ysdeeplearn\deepcode\deeptrain\sp1024\基于人的\3\train_data.mat'
...
load_test_data= r'E:\ysdeeplearn\deepcode\deeptrain\sp1024\基于人的\3\test_data.mat'
...
xs = tf.placeholder(tf.float32, [None, 12288])   # 28x28
...
x_image = tf.reshape(xs, [-1, 12, 1024, 1])
...
## fc1 layer ##
W_fc1 = weight_variable([3*256*64, 1024])
b_fc1 = bias_variable([1024])
# [n_samples, 7, 7, 64] ->> [n_samples, 7*7*64]
h_pool2_flat = tf.reshape(h_pool2, [-1, 3*256*64])
...
def get_batch_data():
    ...
    image_batch, label_batch = tf.train.batch(input_queue, batch_size=512, num_threads=1, capacity=512)
...
#        sess.run(train_step, feed_dict={xs: train_data3, ys:label3,on_train: True, keep_prob: 0.5,pkeep_conv:1})
        if i % 200 == 0:
          print(compute_accuracy(train_data[0:512,:],train_label[0:512,:],norm=False))
