...
def get():
    global idx1
    load_train_label= r'E:\ysdeeplearn\deepcode\deeptrain\何飞\yumu\四类所有\四类\order_hurst_children.mat'
...
#    train_data=np.array(load_matrix,dtype='float32')
    train_data=np.load(r'E:\ysdeeplearn\deepcode\deeptrain\何飞\yumu\data.npy')
#    
#    train_data=np.load('E:\\ysdeeplearn\\deepcode\\deeptrain\\何飞\yumu\\data_rf_children.npy')
...
#    train_data,train_label,test_data,test_label=get()
    with tf.Graph().as_default(): 
        ...
        param_dict = {
                ...
                'n_inputs' : 60,#train_data_width,   # data input 
    ...
    #    choice_num_not = param_dict['choice_num_not']
        ...
        _x = tf.placeholder(tf.float32, [None, 720])
...
#            'out1': tf.Variable(tf.random_normal([n_hidden_units, 512]))
        }
        biases = {
            # (128, )
...
#            'out1': tf.Variable(tf.constant(0.1, shape=[512, ]))
        }
...
#            results = tf.matmul(results1, weights['out']) + biases['out'] 
...
#                        saver.save(sess,'ckptcepstrum/mnist.ckpt')
            else:
                train_data=np.load(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\train_data.npy')
                test_data=np.load(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\test_data.npy')

                test_label=np.load(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\test_label.npy')
                train_label=np.load(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\train_label.npy')
                model_file=tf.train.latest_checkpoint('ckptcepstrum/')
                saver.restore(sess,model_file)
                t= sess.run(outputs[-1], feed_dict={_x: train_data, y: train_label, keep_prob: 1, batch_size: train_data.shape[0]})
                t1= sess.run(outputs[-1], feed_dict={_x: test_data, y: test_label, keep_prob: 1, batch_size: test_data.shape[0]})
                t2= sess.run(accuracy, feed_dict={_x: test_data, y: test_label, keep_prob: 1, batch_size: test_data.shape[0]})
                np.save(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\traindatatezheng.npy',t)
                np.save(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\trainlabeltezheng.npy',train_label)
                np.save(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\testdatatezheng.npy',t1)
                np.save(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\testlabeltezheng.npy',test_label)
        ...
        np.save(new2,s)
        np.save(new3,s1)
        np.save(new4, test_label)
        np.save(new5,idx1)
        np.save(new6,test_index)
        
#        np.save(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\train_data',train_data)
#        np.save(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\train_label',train_label)
#        np.save(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\test_data',test_data)
#        np.save(r'E:\ysdeeplearn\deepcode\deeptrain\RNN训练完的特征\cepstrum\test_label',test_label)
        if jj==1:
            break